...
sfu_3by5 <- function(rseed, ntrial, p.true.tox, p.true.eff, target.tox, target.eff, samplesize, cohortsize, 
      ...
      modelstring <- "
        model {
          ...
          for(i in 1:7){
            ...
            # theta.e[i] ~ dt(a.e[i],1/b.e[i],1)T(-100,100) # sensitivity analysis 4
          }
        }
        "
      jags.data <- list(tox=c(tox[1,],tox[2,],tox[3,]), eff=c(eff[1,],eff[2,],eff[3,]), n=c(n[1,],n[2,],n[3,]),
...
# 
# sfu_3by5(rseed=1, ntrial=10, p.true.tox, p.true.eff, target.tox, target.eff, samplesize, cohortsize, 
#          assessment.window, accrual.rate, u11, u00)
