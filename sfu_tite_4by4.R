...
sfu_tite_4by4 <- function(rseed, ntrial, p.true.tox, p.true.eff, target.tox, target.eff, samplesize, cohortsize, 
  ...
  model.fit <- function(tox.event, eff.event, dv, t.decision, t.enter, n,
    ...
    post.prob.u <- matrix(colMeans((100*(1-t.samp)*e.samp+u11*t.samp*e.samp+u00*(1-t.samp)*(1-e.samp))>u.bench),nrow=4, byrow=T)
  ...
  locationM <- matrix(1:16,ncol=dim(p.true.tox)[1],byrow=T)
  durationV <- NULL
  niters=2000
  ...
  for(trial in 1:ntrial){
      ...
      for(runt in 1:dim(traceRunin)[1]){
        ...
        for(j in 1:cohortsize){
          if(j==1) { t.enter = c(t.enter, t.decision) }else {
            t.enter = c(t.enter, t.enter[length(t.enter)] + runif(1, 0, 2*accrual.rate/cohortsize))} # uniform
        }
        ...
        while(pending==1)
        {
          ...
          t.decision = t.decision + runif(1, 0, 2*accrual.rate/cohortsize)#update decision time
          ...
          if(n.pend>sum(cset)*maxpen) {pending=1}else{
            ...
            if(tox[d[1],d[2]]>0 | eff[d[1],d[2]]>0 | runt == dim(traceRunin)[1]){
              ...
              # tox.monitor <- (post.prob.overtox<cutoff.eli.T2)
    ...
    for(pp in 1:ncohort2){
      ...
      for(j in 1:cohortsize){
        ...
        else {t.enter = c(t.enter, t.enter[length(t.enter)] + runif(1, 0, 2*accrual.rate/cohortsize))} # uniform
      }
      ...
      while(pending==1)
      {
        ...
        if(pp==ncohort2) { t.decision = t.decision + max(assessment.window)}else {
          t.decision = t.decision + runif(1, 0, 2*accrual.rate/cohortsize)
        }
...
# 
# sfu_tite_4by4(rseed=1, ntrial=10, p.true.tox, p.true.eff, target.tox, target.eff, samplesize, cohortsize, 
#               assessment.window, accrual.rate, u11, u00, maxpen)
